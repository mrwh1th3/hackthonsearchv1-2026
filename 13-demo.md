...
- **"¿Por qué esta sí y aquella no?"** Después del dictamen, abrir el panel `Contraste`: el RFC comparable con resultado distinto y la razón tipificada. Ese es el minuto que responde literalmente la pregunta del juez. **Comprobar que dispara antes de prometerlo:** el comparable sale de `forense.v_contraste_caso` (db/018 + db/020), que busca primero un vecino del mismo cluster con nivel menos grave y al menos una pista en común. Sobre los datos que existen hoy devuelve cero filas, porque las corridas de prueba tienen evidencia de cinco filas para quince casos y ninguna razón se puede tipificar. Necesita una corrida con evidencia repartida entre varios RFC, o sea el expediente real con API. Si en el ensayo no dispara, no abrir el panel vacío: usar el caso del despacho contable de 2:45, que responde la misma pregunta con la defensa.
