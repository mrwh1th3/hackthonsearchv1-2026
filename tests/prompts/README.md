...
techos, léxico, few-shot, trayectoria, reintentos y prompt injection sobre `n8n/prompts`.

```bash
npm ci --prefix contracts --ignore-scripts     # los fixtures válidos salen de contracts
node --test "tests/prompts/*.test.mjs"         # 96 pruebas (incluye el check del manifest)
...
| `reintento.test.mjs` | Motivo tipificado por intento: cambia el texto, no el contrato. |
| `techos.test.mjs` | Techos de paquete y de system, ámbito por defecto y truncado por bloques. |
