 ...
 * Se muestran en su lugar nombre/fecha de corte/estado (reales) y el badge
...
export function CorridaPicker({ corridas, onSelect }: CorridaPickerProps) {
  ...
  return (
    <div className="flex w-full flex-col items-center">
      <section aria-label="Choose an investigation dataset" className="insp-surface flex w-full max-w-[700px] flex-col gap-4 p-4 sm:p-5">
        <div className="flex items-stretch gap-3 max-sm:flex-col">
          ...
          >
            ...
            <span className="text-[15px] font-medium text-text">Upload data</span>
            <span className="text-[12.5px] leading-relaxed text-text-subtle">SQLite · CSV · Excel · ZIP</span>
          </button>
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-2.5">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {orden === "reciente" ? "Newest first" : "Oldest first"}
              </button>
            </div>
            ...
            <div className="flex max-h-[196px] flex-col gap-1.5 overflow-y-auto pb-3.5 pr-1 [scrollbar-width:thin]">
              {visibles.map((c) => (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1"><span className="block truncate text-[13px] font-medium text-text">{c.nombre}</span><span className="mt-0.5 block truncate text-[10.5px] text-text-subtle">{soloFecha(c.fecha_corte)}</span></span>
                  ...
                  <span className="flex-none rounded-pill bg-surface-muted px-2 py-0.5 text-[10.5px] text-text-subtle">{({ lista: "Ready", completada: "Completed", auditada: "Audited", procesando: "Processing", cargando: "Uploading", error: "Error" } as Record<string, string>)[c.estado] ?? c.estado.replaceAll("_", " ")}</span>
                </button>
              ))}
              {visibles.length === 0 && corridas.length > 0 && (
                <p className="py-5 text-center text-[13px] text-text-subtle">No dataset matches &ldquo;{query}&rdquo;.</p>
              )}
              {corridas.length === 0 && (
                <p className={cn("py-5 text-center text-[13px] text-text-subtle")}>No datasets yet.</p>
              )}
            </div>
