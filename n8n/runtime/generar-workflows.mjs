...
export function investigarCluster() {
  ...
  add(sql(
    ...
    [
      ...
      '       0 AS intento',
      "  FROM jsonb_to_record(forense.crear_caso($1::uuid, $2::uuid, 'pipeline', $3::text,",
      '                                          $4::text, $5::text))',
      '    AS c(ok boolean, creado boolean, caso_id uuid, cluster_id uuid, corrida_id uuid)',
    ].join('\n'),
    "={{ $json.corrida_id }}, ={{ $json.cluster_id }}, ={{ $('Normalizar entrada').first().json.valor_untrusted }}, ={{ $json.idempotency_key }}, ={{ $execution.id }}, ={{ $json.investigacion_id }}",
  ...
  add(sql(
    'Contexto ronda 1',
    [
      'SELECT x.caso_id, x.cluster_id, x.corrida_id, $2::uuid AS investigacion_id,',
      '       x.context_hash, x.version_contexto,',
      ...
      '    AS x(ok boolean, caso_id uuid, cluster_id uuid, corrida_id uuid,',
      '         context_hash text, version_contexto int, familias_evaluables text[])',
    ].join('\n'),
  ...
  add(sql(
    ...
    [
      ...
      "       (x.value->>'tarea_id')::uuid AS tarea_id,",
      "       ARRAY(SELECT (e.value->>'tarea_id')::uuid",
      ...
      '  FROM jsonb_to_record(forense.crear_tareas_ronda($1::uuid, 1, $2::text[], 0))',
      '    AS t(ok boolean, caso_id uuid, ronda int, intento int, tareas jsonb,',
      '         version_contexto int),',
      '       jsonb_array_elements(t.tareas) AS x',
    ].join('\n'),
  ...
  add(sql(
    ...
    [
      ...
      "       (x.value->>'tarea_id')::uuid AS tarea_id,",
      "       ARRAY(SELECT (e.value->>'tarea_id')::uuid",
      '               FROM jsonb_array_elements(r.tareas) e) AS tarea_ids,',
      '       r.version_contexto,',
      "       (now() + interval '15 minutes') AS deadline",
      '  FROM jsonb_to_record(forense.expandir_y_crear_tareas_r2($1::uuid, $2::text[], $3::text[]))',
      '    AS r(ok boolean, expandidos text[], tareas jsonb, version_contexto int, motivo text),',
      '       jsonb_array_elements(r.tareas) AS x',
    ].join('\n'),
