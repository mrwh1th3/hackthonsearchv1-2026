// n8n/runtime/nodos/interpretar-respuesta.mjs — fuente del Code node
// «Interpretar respuesta» del worker (17 §5.5 a §5.8).
//
// Clasifica `stop_reason`, extrae la cola de `tool_use` con sus IDs y, en
// `end_turn`, parsea el JSON de salida.
//
// LÍMITE DECLARADO: un Code node de n8n no puede cargar el validador de
// `contracts/` (ajv). Aquí solo se comprueba la ESTRUCTURA mínima del rol
// (claves requeridas y ausencia de claves extra) y se marca
// `requiere_validacion_contrato: true`. La validación autoritativa contra el
// schema la hace el backend antes de persistir: una salida que pasa este nodo
// todavía NO es una salida válida.
//
// Nunca se ejecutan herramientas mencionadas en el texto, y el JSON válido no
// prueba IDs, permisos ni hechos (17 §5.6).

export function interpretarRespuestaNodo(x) {
  // <<<CODE_NODE_INICIO
  const REQUERIDAS = {
