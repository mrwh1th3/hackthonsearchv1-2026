...
- **1.2.0 (2026-09-12, H3):** (a) `common.referencia` admite el prefijo `CADENA` (R2 produce ciclos y cadenas; hasta ahora viajaban como `CICLO:cadena:…`, que sigue siendo válido). (b) `runtime.contexto`: los paquetes de cierre (Auditor/Defensor/Réplica/Redactor/Editor) admiten `trayectoria` opcional: serie mensual `{mes, facturado, recibido, nomina, n_cfdi, n_movimientos, eventos[]}` calculada por SQL (`forense.v_trayectoria_rfc`) para la sección Trayectoria de 21 §2; el Redactor no la inventa. (c) `product.evento_forense.tipo_evento` añade `corrida_cargada` e `inyeccion` (05/21). Todo aditivo.
- **1.2.1 (2026-09-12, H5):** `product.evento_forense.tipo_evento` añade `paso_en_cola` y `paso_checkpoint` (eventos técnicos del worker de runtime, 17 §3). forense-db los añade al check de bitácora en migración aditiva.
- **1.3.0 (2026-09-12, H8):** `editor.descartar`/`editor.descartado`, `editor.revertir`, `editor.borrador`/`editor.borrador_guardado`, `editor.exportar`/`editor.exportado`: las cuatro operaciones del BFF del editor que antes vivían como zod local (propuesta de forense-editor en `web/lib/document/contratos-propuestos.json`, convertida a las convenciones del release: refs a `common.uuid/hash/fecha` y `editor.documento`). Aditivo.
- **1.3.1 (2026-09-12, H9):** `editor.descartado` admite `bitacora` opcional (regla 2: la respuesta declara si dejó evento). Aditivo.
- **1.4.0 (2026-09-12, H11):** `product.contraste`: el DTO del panel **Contraste** (21 §"Por qué esta sí y aquella no"), que responde la pregunta literal del juez principal. La UI ya lo renderizaba contra fixture y `SupabaseDataSource.getContraste` devolvía `null` porque no existía la vista SQL; el contrato se publica ANTES para que la vista de db y el lector de la webapp no divergan. `razon_tipificada` es un enum cerrado de tres valores derivables de datos, no texto libre: la razón la calcula SQL, no un modelo (regla 4). `explicacion` tiene tope de 600 caracteres y prohíbe texto del contribuyente (regla 6). Aditivo.
