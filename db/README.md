...
| `018_contraste.sql` | `v_contraste_caso(p_caso uuid) returns table(...)`: el panel **Contraste** que pidió el juez ("por qué esta sí y aquella no", 21 §2). Devuelve el RFC comparable más cercano de la MISMA corrida (regla 10) con el mismo giro, al menos una pista en común y nivel estrictamente menos grave, más la `razon_tipificada` de su exoneración (`cobertura_insuficiente` > `defensa_aceptada` > `familia_faltante`, en el orden en que decide `n8n/runtime/auditor-final.mjs:50-55`). Cero filas si no hay comparable o si ninguna razón aplica: no se inventa una fila (regla 4). Sin texto del contribuyente (regla 6). Permisos y modificadores copiados de `v_trayectoria_rfc`. | hecho |
| `019_pista_t2.sql` | Recalibra la pierna (a) de `pista_t2` (sincronía): ventana de 6 h → 15 min, la escala de un lote de timbrado, con guarda relativa a los pares del giro y guarda para snapshots sin resolución intradía. Con la ventana de 6 h marcaba 85 de 100 RFC con precisión 7.1%, por debajo de la tasa base de fraude; con 15 min marca 13 con precisión 38.5%. Sobre gen-v1 (hora plana) sigue `no_evaluable` con motivo en bitácora. | hecho |
| `020_contraste_cluster.sql` | `create or replace` de `v_contraste_caso` con la MISMA firma y las mismas siete columnas: el comparable se busca primero DENTRO del cluster (`casos.resultado_por_rfc`, que emite `n8n/runtime/auditor-final.mjs` desde a708225) y sólo si ahí no hay se cae al emparejamiento caso-a-caso de 018. Nivel de referencia: el del `rfc_principal` en `resultado_por_rfc` y, si falta, `casos.nivel` (precedente de `002_views.sql:1026-1038`). `familia_faltante` cuenta familias sobre los `evidencia_ids` que enumera el propio elemento, así que SQL no puede discrepar del runtime. Cero filas si ninguno de los dos caminos da comparable. | hecho |
